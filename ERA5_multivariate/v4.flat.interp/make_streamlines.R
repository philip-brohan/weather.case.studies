...
Imagedir<-sprintf("%s/images/ERA4v_multivariate",Sys.getenv('SCRATCH'))
...
make.streamlines<-function(year,month,day,hour,Options,streamlines=NULL) {
    ...
    uwnd<-ERA4v.get.slice.at.hour('uwnd.10m',year,month,day,hour)
    vwnd<-ERA4v.get.slice.at.hour('vwnd.10m',year,month,day,hour)
    t.actual<-ERA4v.get.slice.at.hour('air.2m',year,month,day,hour)
    t.normal<-t.actual
