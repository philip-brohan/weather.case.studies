...
Imagedir<-sprintf("%s/images/TWCR_multivariate.V3vV2c",Sys.getenv('SCRATCH'))
...
plot.hour.V3<-function(year,month,day,hour,streamlines) {
    ...
    t2m<-get.member.at.hour('air.2m',year,month,day,hour,member,version='4.1.8')
    ...
    prmsl.T$data[]<-as.vector(prmsl.T$data)-as.vector(prn$data)

    icec<-get.member.at.hour('icec',year,month,day,hour,member,version='4.1.8')
    prate<-get.member.at.hour('prate',year,month,day,hour,member,version='4.1.8')
