...
Imagedir<-'.'
Stream.dir<-sprintf("%s/images/TWCR_multivariate.mean.V3",Sys.getenv('SCRATCH'))
...
Options<-WeatherMap.set.option(Options,'fog.min.transparency',0.90)
...
plot.hour<-function(year,month,day,hour,streamlines) {
    ...
    image.name<-sprintf("V3.mfm.%04d-%02d-%02d:%02d.%02d.pdf",year,month,day,as.integer(hour),
    ...
    prn<-GSDF.select.from.1d(prn,'time',1)
