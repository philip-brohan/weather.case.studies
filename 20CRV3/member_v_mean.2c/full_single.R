...
Imagedir<-sprintf("%s/images/TWCR_multivariate.m_v_m.V2c.nf",Sys.getenv('SCRATCH'))
Stream.dir.V2c<-sprintf("%s/images/TWCR_multivariate.V2c",Sys.getenv('SCRATCH'))
Stream.dir.mean.V2c<-sprintf("%s/images/TWCR_multivariate.mean.V2c",Sys.getenv('SCRATCH'))
    ...
    popViewport()
     ...
     pushViewport(viewport(x=unit(0.25,'npc'),y=unit(0.5,'npc'),
       ...
       grid.polygon(x=unit(c(0,1,1,0),'npc'),
                    y=unit(c(0,0,1,1),'npc'),
                    gp=gpar(fill=Options$sea.colour))
       s<-get.streamlines(opt$year,opt$month,opt$day,opt$hour,Stream.dir.mean.V2c)
       plot.hour.mean.V2c(year,month,day,hour,s)
...
plot.hour.V2c<-function(year,month,day,hour,streamlines) {
    ...
    icec<-TWCR.get.slice.at.hour('icec',year,month,day,hour,version=opt$version)
...
plot.hour.mean.V2c<-function(year,month,day,hour,streamlines) {
