...
while(current.day<=end.day) {
  ...
  if(n.new.jobs>100) {
      for(hour in seq(0,23)) {
          sink('V2c.mvm.multivariate.nf.slm')
          cat('#!/bin/ksh -l\n')
          cat('#SBATCH --output=/scratch/hadpb/slurm_output/TWCR_multivariate.V2c.mvm-%j.out\n')
          ...
          fn<-NULL
           for(min in seq(0,0.75,0.25)) {
             fn<-sprintf("%s/images/TWCR_multivariate.m_v_m.V2c.nf/%04d-%02d-%02d:%02d.%02d.png",
          ...
          sink()
          if(needed>0) {
             system('sbatch V2c.mvm.multivariate.nf.slm')
           }
          unlink('V2c.mvm.multivariate.nf.slm')
      }
