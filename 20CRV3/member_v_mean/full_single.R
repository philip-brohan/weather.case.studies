...
Imagedir<-sprintf("%s/images/TWCR_multivariate.m_v_m.nf",Sys.getenv('SCRATCH'))
Stream.dir.V3<-sprintf("%s/images/TWCR_multivariate.V3",Sys.getenv('SCRATCH'))
Stream.dir.mean.V3<-sprintf("%s/images/TWCR_multivariate.mean.V3",Sys.getenv('SCRATCH'))
    ...
    popViewport()
     ...
     pushViewport(viewport(x=unit(0.25,'npc'),y=unit(0.5,'npc'),
       ...
       grid.polygon(x=unit(c(0,1,1,0),'npc'),
                    y=unit(c(0,0,1,1),'npc'),
                    gp=gpar(fill=Options$sea.colour))
       s<-get.streamlines(opt$year,opt$month,opt$day,opt$hour,Stream.dir.mean.V3)
       plot.hour.mean.V3(year,month,day,hour,s)
...
plot.hour.mean.V3<-function(year,month,day,hour,streamlines) {
