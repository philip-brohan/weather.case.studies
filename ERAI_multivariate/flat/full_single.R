...
Stream.dir<-sprintf("%s/images/ERAI_multivariate",Sys.getenv('SCRATCH'))
...
plot.hour<-function(year,month,day,hour,streamlines) {
    ...
    if(hour.fraction>0) {
      ...
      t2n2<-readRDS(sprintf("%s/ERA_Interim/climtologies.test/air.2m.%02d.Rdata",
    ...
    prate<-ERAI.get.slice.at.hour('prate',year,month,day,hour)
