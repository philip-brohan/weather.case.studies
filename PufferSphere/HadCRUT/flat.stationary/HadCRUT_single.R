...
# Flat projection version - fixed pole
...
Imagedir<-sprintf("%s/images/HadCRUT4.red_blue.flat.fixed",Sys.getenv('SCRATCH'))
...
Draw.temperature<-function(temperature,Options) {
...
plot.field<-function(field,land,year,month,idx) {    
    ...
    Options$label<-sprintf("%04d-%02d",year,month)
      ...
      Draw.temperature(field,Options)
