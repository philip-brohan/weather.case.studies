...
while(current.day<=end.day) {
  ...
  if(n.new.jobs>30) {
    ...
    while(step<=12) {
      ...
      cat('#!/bin/ksh -l\n')
      cat('#SBATCH --output=/scratch/hadpb/slurm_output/HadCRUT-%j.out\n')
      ...
      while(step<=12 && this.job<per.job) {
         for(i in seq(1,6)) {
           in.file<-sprintf("%s/images/HadCRUT4.red_blue/%04d-%02d.%02d.png",
                            Sys.getenv('SCRATCH'),year(current.day),
                            month(current.day),i)
           out.file<-sprintf("%s/images/HadCRUT4.red_blue.stom/%04d-%02d.%02d.png",
                            Sys.getenv('SCRATCH'),year(current.day),
                            month(current.day),i)
