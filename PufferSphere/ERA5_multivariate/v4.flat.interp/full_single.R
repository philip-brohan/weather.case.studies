...
Stream.dir<-sprintf("%s/images/ERA4v_multivariate",Sys.getenv('SCRATCH'))
...
plot.hour<-function(year,month,day,hour,streamlines) {
    ...
    t2m<-ERA4v.get.slice.at.hour('air.2m',year,month,day,hour)
    ...
    prmsl.T<-ERA4v.get.slice.at.hour('prmsl',year,month,day,hour)
    ...
    icec<-ERA4v.get.slice.at.hour('icec',year,month,day,hour)
    prate<-ERA4v.get.slice.at.hour('prate',year,month,day,hour)
