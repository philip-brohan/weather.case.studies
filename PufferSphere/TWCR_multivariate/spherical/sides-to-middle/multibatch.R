  ...
  if(n.new.jobs>30) {
    ...
    while(step<=12) {
      ...
      while(step<=12 && this.job<per.job) {
         for(i in seq(0,24)) {
           in.file<-sprintf("%s/images/TWCR_multivariate_spherical/%04d-%02d-%02d:%02d.png",
                            Sys.getenv('SCRATCH'),year(current.day),
                            month(current.day),day(current.day),i)
           out.file<-sprintf("%s/images/TWCR_multivariate_spherical.stom/%04d-%02d-%02d:%02d.png",
                            Sys.getenv('SCRATCH'),year(current.day),
                            month(current.day),day(current.day),i)
