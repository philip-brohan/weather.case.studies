...
# D-Day - meteorology only - no background
...
Imagedir<-sprintf("%s/images/d-day_nobg",Sys.getenv('SCRATCH'),version)
...
# Remove all the land polygons
land$x<-land$x[1:4]
land$y<-land$y[1:4]
land$lakes$x<-land$x[1:4]
land$lakes$y<-land$y[1:4]
...
Options$land.colour<-rgb(1,1,1,1)
Options$sea.colour<-rgb(1,1,1,1)
...
make.streamlines<-function(year,month,day,hour,streamlines=NULL) {
    ...
    t.normal<-t.actual
    t.normal$data[]<-rep(288,length(t.normal$data))
...
for(n.count in seq(0,n.total)) {
    ...
    if(n.count%%22==0) mccollect(wait=TRUE)
