...
Stream.dir<-sprintf("%s/images/UK_winter_2015.ERA5",Sys.getenv('SCRATCH'))
...
Options<-WeatherMap.set.option(Options,'wind.vector.move.scale',1.5)
Options<-WeatherMap.set.option(Options,'wind.vector.density',10)
...
plot.hour<-function(year,month,day,hour,streamlines) {
    ...
    t2m<-ERA5.get.slice.at.hour('air.2m',year,month,day,hour,type='mean')
    t2n<-ERA5.get.slice.at.hour('air.2m',year,month,day,hour,type='normal')
