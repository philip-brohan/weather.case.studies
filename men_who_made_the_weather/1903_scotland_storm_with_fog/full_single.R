...
Imagedir<-sprintf("%s/images/1903_scotland_storm_fog",Sys.getenv('SCRATCH'))
...
get.spread.at.hour<-function(variable,year,month,day,hour,version='3.5.1') {

       t<-TWCR.get.members.slice.at.hour(variable,year,month,day,
                                  hour,version=version)
       t<-GSDF.reduce.1d(t,'ensemble',sd)
       gc()
       return(t)
  }
...
plot.hour<-function(year,month,day,hour,streamlines) {
    ...
    prm<-get.mean.at.hour('prmsl',year,month,day,hour,version='3.5.1')
    prn<-TWCR.get.slice.at.hour('prmsl',year,month,day,hour,type='normal',version='3.4.1')
    prn<-GSDF.regrid.2d(prn,prmsl.T)
    prmsl.spread<-get.spread.at.hour('prmsl',year,month,day,hour,version='3.5.1')
    prmsl.spread<-GSDF.regrid.2d(prmsl.spread,prmsl.T)
