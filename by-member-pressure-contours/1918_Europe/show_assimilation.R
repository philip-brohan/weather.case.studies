...
Day<-01
Hour<-0
d.total<-10 # Number of days to be rendered
version<-'3.5.1'
members<-seq(1,56)
...
get.assimilation.step.start<-function(year,month,day,hour) {
  return(get.forecast.step.end(year,month,day,hour))
}
get.assimilation.step.end<-function(year,month,day,hour) {
  return(get.forecast.step.start(year,month,day,hour))
}
