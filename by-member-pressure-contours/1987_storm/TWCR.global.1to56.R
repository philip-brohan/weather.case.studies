...
# 1987 Jan  add members one at a time
...
Imagedir<-sprintf("%s/images/1987-global-1to56",Sys.getenv('SCRATCH'))
...
Options$mslp.step=250*2                       # Smaller -more contours
Options$mslp.tpscale=3500/2                    # Smaller -contours less transparent
...
plot.hour<-function(year,month,day,hour) {    
    ...
    image.name<-sprintf("%02d.png",length(members))
...
for(n.count in seq(1,56)) {
    ...
    image.name<-sprintf("%02d.png",length(members))
    ifile.name<-sprintf("%s/%s",Imagedir,image.name)
    if(file.exists(ifile.name) && file.info(ifile.name)$size>0) {
      members<-seq(1,n.count+1)
      next
    }
