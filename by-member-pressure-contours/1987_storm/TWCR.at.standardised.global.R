...
# 1987 annual - standardised air temperatures
...
Day<-2
Hour<-0
n.total<-0#24*364 # Total number of timesteps to be rendered
version<-'3.5.1'
fog.threshold<-exp(1)
...
Imagedir<-sprintf("%s/images/1987-global-at-standardised",Sys.getenv('SCRATCH'),version)
...
Options$vp.lon.min<- -175
Options$vp.lon.max<-  175
...
Options$mslp.range=5                    # Anomaly for max contour
Options$mslp.step=1                       # Smaller -more contours
...
plot.hour<-function(year,month,day,hour) {    
    ...
    Options$label<-sprintf("%04d-%02d-%02d:%02d",year,month,day,as.integer(hour))
      ...
      WeatherMap.draw.obs(obs,Options)
       prmsl.normal<-TWCR.get.slice.at.hour('air.2m',year,month,day,hour,version='3.4.1',
                                               type='normal')
       prmsl.sd<-TWCR.get.slice.at.hour('air.2m',year,month,day,hour,version='3.4.1',
                                               type='standard.deviation')
       e<-TWCR.get.members.slice.at.hour('air.2m',year,month,day,
                                  hour,version=version)
      m<-GSDF.select.from.1d(e,'ensemble',1)
      prmsl.normal<-GSDF.regrid.2d(prmsl.normal,m)
      prmsl.sd<-GSDF.regrid.2d(prmsl.sd,m)
      for(vn in seq_along(members)) {
            m<-GSDF.select.from.1d(e,'ensemble',vn)
  	    m$data[]<-(as.vector(m$data)-as.vector(prmsl.normal$data))/as.vector(prmsl.sd$data)
