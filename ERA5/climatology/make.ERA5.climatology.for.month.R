...
# This script runs make.ERA5.climatology.at.point.R repeatedly to make all the
...
while(c.time<e.time) {
  cmd<-sprintf("./make.ERA5.climatology.at.point.R --variable=%s --month=%d --day=%d --hour=%d",
               opt$variable,
               as.integer(lubridate::month(c.time)),
               as.integer(lubridate::day(c.time)),
               as.integer(lubridate::hour(c.time)))
  system(cmd)
  c.time<-c.time+lubridate::hours(1)
}
